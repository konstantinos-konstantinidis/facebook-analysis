...
folder_path='E:\\facebook-konstantinoskonstantinidis129'
...
for root,dirs,files in os.walk(path):
    name=root.split('\\')
    if len(name)>=(4+0):                                #!!!!!!! +number of folders before username
        name_list.append(name[3+0])
...
#    json.loads(dataz).encode('latin1').decode('utf8')
    jfile.close
    ...
    for i in range(0,n):
        ...
        try:
            if dataz['messages'][i]['sender_name']=='Konstantinos Konstantinidis':      #!!!!!!!!!!!!!!!!!
                mymsg +=1
                mine.append('user')
                mw=mw+len(dataz['messages'][i]['content'].split(' '))
            else:
                mine.append('others')
                ow=ow+len(dataz['messages'][i]['content'].split(' '))
            temp=dataz['messages'][i]['timestamp_ms']
            temp=int(temp-temp%1000)/1000
            ymdh=datetime.datetime.fromtimestamp(temp)
...
total_words=(numpy.array(my_words)+numpy.array(ot_words)).tolist()

final_msg=[['conversation name','total messages','my messages','total words','my words','msg %','words %']]
for row in range(1,len(my_msg)+1):
    try:
        mmm=my_msg[row-1]/tot_msg[row-1]
    except:
        mmm=0
    try:
        www=my_words[row-1]/total_words[row-1]
    except:
        www=0
    final_msg.append([name_list2[row-1],tot_msg[row-1],my_msg[row-1],total_words[row-1],my_words[row-1],mmm,www])
